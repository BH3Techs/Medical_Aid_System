...
import { getEntities, reset } from './currency.reducer';
...
export const Currency = () => {
  ...
  const links = useAppSelector(state => state.currency.links);
  const entity = useAppSelector(state => state.currency.entity);
  ...
  return (
    <div>
      <h2 id="currency-heading" data-cy="CurrencyHeading">
        <Translate contentKey="medicalAidSystemApp.currency.home.title">Currencies</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="medicalAidSystemApp.currency.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/currency/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="medicalAidSystemApp.currency.home.createLabel">Create new Currency</Translate>
          </Link>
        </div>
      </h2>
      <div className="table-responsive">
        <InfiniteScroll
          dataLength={currencyList ? currencyList.length : 0}
          next={handleLoadMore}
          ...
          {currencyList && currencyList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="medicalAidSystemApp.currency.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  ...
                  <th className="hand" onClick={sort('active')}>
                    <Translate contentKey="medicalAidSystemApp.currency.active">Active</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
                    <Translate contentKey="medicalAidSystemApp.currency.plans">Plans</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {currencyList.map((currency, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{currency.plans ? <Link to={`/plans/${currency.plans.id}`}>{currency.plans.id}</Link> : ''}</td>
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="medicalAidSystemApp.currency.home.notFound">No Currencies found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
